...
public class GPGMultiplayerManager : AbstractManager
{
	...
	{
		private GPGRoom _room_k__BackingField;
		...
		public GPGRoomUpdateStatus status
		{
			get
			{
				return (GPGRoomUpdateStatus)this.statusCode;
			}
		}
	}
	...
	{
		GPGMultiplayerManager.GPGRoomUpdateInfo gPGRoomUpdateInfo = Json.decode<GPGMultiplayerManager.GPGRoomUpdateInfo>(json, null);
	...
	{
		GPGMultiplayerManager.onRoomAutoMatchingEvent.fire(Json.decode<GPGRoom>(json, null));
	}

	private void onRoomConnecting(string json)
	{
		GPGMultiplayerManager.onRoomConnectingEvent.fire(Json.decode<GPGRoom>(json, null));
	}
}
