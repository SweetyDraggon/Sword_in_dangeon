...
public class LeanTween : MonoBehaviour
{
		...
		if (LeanTween.frameRendered != Time.frameCount)
		{
			...
			while (num < LeanTween.tweenMaxSearch && num < LeanTween.maxTweens)
			{
				if (LeanTween.tweens[num].toggle)
				{
					...
					LeanTween.timeTotal = LeanTween.tween.time * Time.timeScale;
					...
					else
					{
						...
						if (LeanTween.tween.passed + LeanTween.dt > LeanTween.timeTotal && LeanTween.tween.direction > 0f)
						{
							...
							LeanTween.tween.passed = LeanTween.timeTotal;
						}
						...
						if (((double)LeanTween.tween.passed == 0.0 && (double)LeanTween.tweens[num].delay == 0.0) || (!LeanTween.tween.hasInitiliazed && (double)LeanTween.tween.passed > 0.0))
						{
							...
							case TweenAction.ROTATE_AROUND:
								...
								LeanTween.tween.origRotation = LeanTween.trans.eulerAngles;
								break;
						...
						if (LeanTween.tween.delay <= 0f)
						{
							...
							if ((double)LeanTween.ratioPassed > 1.0)
							{
							...
							if (LeanTween.tweenAction >= TweenAction.MOVE_X && LeanTween.tweenAction <= TweenAction.CALLBACK)
							{
								...
								else if (LeanTween.tweenAction == TweenAction.ROTATE_AROUND)
								{
									...
									LeanTween.trans.RotateAround(LeanTween.trans.TransformPoint(LeanTween.tween.point), LeanTween.tween.axis, angle);
								...
								else if (LeanTween.tweenAction == TweenAction.ALPHA_VERTEX)
								{
									...
									Color32 color = mesh.colors32[0];
									color = new Color((float)color.r, (float)color.g, (float)color.b, LeanTween.val);
								...
								if (obj != null)
								{
									...
									if (LeanTween.tweenAction == TweenAction.VALUE3)
									{
										if (obj.GetType() == typeof(string))
										{
											...
											LeanTween.customTarget.BroadcastMessage(methodName, LeanTween.newVect);
										}
									...
									else if (obj.GetType() == typeof(string))
									{
										...
										if (LeanTween.tween.optional["onUpdateTarget"] != null)
										{
											...
											LeanTween.customTarget.BroadcastMessage(methodName2, LeanTween.val);
										}
										else
										{
											LeanTween.trans.gameObject.BroadcastMessage(methodName2, LeanTween.val);
										}
									}
						...
						if (LeanTween.isTweenFinished)
						{
								...
								else if (LeanTween.tween.optional != null)
								{
									...
									if (LeanTween.tween.optional != null && LeanTween.tween.trans && LeanTween.tween.optional["onComplete"] != null)
									{
										...
										else
										{
											...
											if (action6 == null)
											{
												UnityEngine.Debug.LogWarning("callback was not converted");
											}
										}
									}
									LeanTween.removeTween(num);
									...
									else if (text != string.Empty)
									{
										if (LeanTween.tween.optional["onCompleteTarget"] != null)
										{
											...
											if (obj2 != null)
											{
												LeanTween.customTarget.BroadcastMessage(text, obj2);
											}
											else
											{
												LeanTween.customTarget.BroadcastMessage(text);
											}
										}
										else if (obj2 != null)
										{
											LeanTween.trans.gameObject.BroadcastMessage(text, obj2);
										}
										else
										{
											LeanTween.trans.gameObject.BroadcastMessage(text);
										}
									}
								}
								else
							...
							{
								if (LeanTween.tween.loopCount < 0 && LeanTween.tween.type == TweenAction.CALLBACK)
								{
									if (LeanTween.tween.onComplete != null)
									{
										LeanTween.tween.onComplete();
	...
	public static void cancel(GameObject gameObject, int uniqueId)
	{
		...
		{
			LeanTween.init();
			int num = uniqueId & 16777215;
			int num2 = uniqueId >> 24;
			if (LeanTween.tweens[num].trans == null || (LeanTween.tweens[num].trans.gameObject == gameObject && LeanTween.tweens[num].type == (TweenAction)num2))
			{
				LeanTween.removeTween(num);
			}
		}
	...
	public static void cancel(LTRect ltRect, int uniqueId)
	{
		if (uniqueId >= 0)
		{
			...
			int num = uniqueId & 16777215;
			int num2 = uniqueId >> 24;
			if (LeanTween.tweens[num].ltRect == ltRect && LeanTween.tweens[num].type == (TweenAction)num2)
			{
				LeanTween.removeTween(num);
			}
		}
	...
	private static void cancel(int uniqueId)
	{
		if (uniqueId >= 0)
		{
			...
			int num = uniqueId & 16777215;
			int num2 = uniqueId >> 24;
			if (LeanTween.tweens[num].type == (TweenAction)num2)
			{
				LeanTween.removeTween(num);
			}
		}
	...
	public static LTDescr description(int uniqueId)
	{
		int num = uniqueId & 16777215;
		int num2 = uniqueId >> 24;
		if (LeanTween.tweens[num] != null && LeanTween.tweens[num].uniqueId == uniqueId && LeanTween.tweens[num].type == (TweenAction)num2)
		{
			return LeanTween.tweens[num];
		}
		for (int i = 0; i < LeanTween.tweenMaxSearch; i++)
		{
			if (LeanTween.tweens[i].uniqueId == uniqueId && LeanTween.tweens[i].type == (TweenAction)num2)
			{
	...
	public static void pause(int uniqueId)
	{
		...
		if (LeanTween.tweens[num].type == (TweenAction)num2)
		{
			LeanTween.tweens[num].lastVal = LeanTween.tweens[num].direction;
			LeanTween.tweens[num].direction = 0f;
		}
	}
	...
	public static void pause(GameObject gameObject)
	{
		...
		for (int i = 0; i < LeanTween.tweenMaxSearch; i++)
		{
			if (LeanTween.tweens[i].trans == transform)
			{
				LeanTween.tweens[i].lastVal = LeanTween.tweens[i].direction;
				LeanTween.tweens[i].direction = 0f;
			}
		}
	}
	...
	public static void resume(int uniqueId)
	{
		...
		if (LeanTween.tweens[num].type == (TweenAction)num2)
		{
			LeanTween.tweens[num].direction = LeanTween.tweens[num].lastVal;
		}
	}
	...
	public static void resume(GameObject gameObject)
	{
		...
		for (int i = 0; i < LeanTween.tweenMaxSearch; i++)
		{
			if (LeanTween.tweens[i].trans == transform)
			{
				LeanTween.tweens[i].direction = LeanTween.tweens[i].lastVal;
			}
		}
	}
	...
	public static LTDescr options()
	{
		...
		LeanTween.tween.setId(LeanTween.i);
	...
	{
		return LeanTween.pushNewTween(gameObject, new Vector3(to, 0f, 0f), time, TweenAction.CALLBACK, LeanTween.options().setFrom(new Vector3(from, 0f, 0f)).setOnUpdate(callOnUpdate));
	}

	public static LTDescr value(GameObject gameObject, Action<Vector3> callOnUpdate, Vector3 from, Vector3 to, float time)
	{
		return LeanTween.pushNewTween(gameObject, to, time, TweenAction.VALUE3, LeanTween.options().setFrom(from).setOnUpdateVector3(callOnUpdate));
	}

	public static LTDescr value(GameObject gameObject, Action<float, object> callOnUpdate, float from, float to, float time)
	{
		return LeanTween.pushNewTween(gameObject, new Vector3(to, 0f, 0f), time, TweenAction.CALLBACK, LeanTween.options().setFrom(new Vector3(from, 0f, 0f)).setOnUpdateObject(callOnUpdate));
	}
	...
	private static int pushNewTween(GameObject gameObject, Vector3 to, float time, TweenAction tweenAction, Hashtable optional)
	{
		...
		LeanTween.tween.setId(LeanTween.i);
		LeanTween.tween.hasPhysics = (gameObject.GetComponent<Rigidbody>() != null);
		if (optional != null)
		{
	...
	public static void addListener(GameObject caller, int eventId, Action<LTEvent> callback)
	{
		...
		{
			int num = eventId * LeanTween.LISTENERS_MAX + LeanTween.i;
	...
	public static void dispatchEvent(int eventId, object data)
	{
		...
		{
			int num = eventId * LeanTween.LISTENERS_MAX + i;
